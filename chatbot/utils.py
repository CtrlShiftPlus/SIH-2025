...
def handle_message_in_english(message):
    # Your existing logic to handle messages in English
    # For demonstration, here's a simple echo response
...
def process_query(user_message, language="en"):
    ...
    if language != "en":
        translated_response = translator.translate(response, dest=language)
...
def process_query(query, language="en"):
    query_lower = query.lower()
    response = ""
    response_parts = []  # start with empty list
